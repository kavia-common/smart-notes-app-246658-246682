...
# IMPORTANT: Do NOT exit early; we still want repeatable DB/user/schema initialization.
if sudo -u postgres ${PG_BIN}/pg_isready -p ${DB_PORT} > /dev/null 2>&1; then
    SERVER_ALREADY_RUNNING=1
    echo "PostgreSQL is already running on port ${DB_PORT}."
    echo "Continuing to ensure DB/user/permissions/schema are initialized..."
fi

# Also check if there's a PostgreSQL process running (in case pg_isready fails)
if [ "${SERVER_ALREADY_RUNNING}" -eq 0 ] && pgrep -f "postgres.*-p ${DB_PORT}" > /dev/null 2>&1; then
...
-- Required for gen_random_uuid() defaults used by the app schema
...
# Repeatable schema initialization (safe to run multiple times).
if [ -f "./init_schema.sh" ]; then
    chmod +x ./init_schema.sh
    echo ""
    echo "Initializing Smart Notes schema..."
    ./init_schema.sh
else
    echo "WARNING: init_schema.sh not found; schema was not initialized."
fi
